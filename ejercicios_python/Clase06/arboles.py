...
plt.hist(alturas_jaracaranda, bins=40)
plt.show()

especies = ['Eucalipto', 'Palo borracho rosado', 'Jacarandá']
for esp in especies:
    medidas = np.array(medidas_de_especies(especies=especies, arboleda=arboleda).get(esp))
    plt.scatter(
        x=medidas[:,0], 
        y=medidas[:,1],
        s=60,
...
plt.ylabel("alto (m)")
plt.title("Relación diámetro-alto para cada especie")
plt.show()
